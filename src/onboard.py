...
if __name__ == "__main__":
    ...
    while not rospy.is_shutdown():
            ...
            if packet:
                ...
                if messageType == Message_ID.Mode_Change:
                    ...
                    if not UAV.armed:  # sim UAV
                        ...
                        UAV = DPGA.UAV_Simulator(uav_id, info[4][0], info[4][1], info[4][2], info[2], info[3])
                    UAV.type = info[4][0]
                    UAV.v = info[4][1]
                    UAV.Rmin = info[4][2]
                    waypoint_radius = info[-1]
        ...
        # send data to GCS (u2g communication)
                ...
                if method == WaypointMissionMethod.guide_waypoint:
                    ...
                    if CRPF.method == pathFollowingMethod.path_following_position:
                        ...
                        if UAV.frame_type == FrameType.Quad:
                            desirePoint, index, _, error_of_distance, _ = CRPF.get_desirePoint_withWindow(UAV.v, UAV.local_pose[0], UAV.local_pose[1], UAV.heading*pi/180, index)
                            ...
                            if waypoint_radius == 20:
                                desirePoint, index, _, error_of_distance, desireHeading = CRPF.get_desirePoint_withWindow(UAV.v, UAV.local_pose[0], UAV.local_pose[1], UAV.heading*pi/180, index)
                                if error_of_distance <= 0 and completed:
                                    UAV.set_mode(Mode.LOITER.name)
                                else:
                                    UAV.guide_to_waypoint([desirePoint[0], desirePoint[1], height], desireHeading)
                            else:
                                desirePoint, index, _, error_of_distance, _ = CRPF.get_desirePoint_withWindow(UAV.v, UAV.local_pose[0], UAV.local_pose[1], UAV.heading*pi/180, index)
                                u, pre_error = CRPF.PID_control(UAV.v, UAV.Rmin, UAV.local_pose, UAV.heading*pi/180, desirePoint, pre_error)
                                if error_of_distance <= 0 and completed:
